...
class PlayerView(BaseView):
    ...
    def get(self, request, id):
        ...
        for table in sorted({game.table for game in games}, key=attrgetter('name')):
            ...
            game['scores'] = sorted([{'week': g.group.week, 'group': g.group, 'score': g.score, 'league_points': g.league_points} for g in games if g.table == table], key=itemgetter('score'), reverse=True)
